...
//          .whereArrayContains("selectedBranch", branchID)
    ...
    db.whereEqualTo("userID", userID)
      ...
      .addSnapshotListener { querySnapshot, firebaseFirestoreException ->
        if (firebaseFirestoreException != null) {
          ...
          Log.d(TAG, "Fetch redeemed coupon error $firebaseFirestoreException")
        } else {
  ...
  private fun updateStoreListHeaders() {
    ...
    this.couponList.forEach { coupon ->
      val newAdapter = AdapterItem(coupon, AllCouponListAdapter.TYPE_COUPON)
      formattedCouponData.add(newAdapter)
    }
    updateData()
